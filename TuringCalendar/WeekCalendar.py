...
OUTPUT = 'backgrounds/turing_week{}.jpg'  # 生成的壁纸路径
...
for week in range(1, 54):
    with Image(filename=PDF_SOURCE.format(PAGE_OFFSET + week), resolution=200) as calendar:
        with Image(filename=BACKGROUND_SOURCE) as background:
            background.composite_channel('default_channels', calendar, 'blend', background.page_width - MARGIN_RIGHT,
                                         MARGIN_TOP)
            background.save(filename=OUTPUT.format(week))
